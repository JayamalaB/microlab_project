...
exports.verifyOtp = async (req, res) => {
  try {
    ...
    let rows;
    if (isDemo) {
      // Look up by mobile only — skip OTP and expiry check
      [rows] = await db.query(
        `SELECT user_id, client_id, user_name, user_microlab_type, user_mobile_no
         FROM ip_users WHERE user_mobile_no = ? AND user_active = 1`,
        [mobile]
      );
    ...
    // Clear OTP after use — skipped for demo account so the fixed OTP remains reusable.
    // is_logged_in is deliberately NOT set here — it's only set true once the
    // single-active-session claim below actually succeeds.
    if (!isDemo) {
      await db.query(
